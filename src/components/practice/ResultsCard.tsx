...
function parseSections(text: string): FeedbackSection[] | null {
  // Match **Title:** pattern
...
export function ResultsCard({ results, isAiLoading, onReset }: ResultsCardProps) {
  ...
  const sections = results.aiFeedback ? parseSections(results.aiFeedback) : null
  ...
  return (
    <div className="results-panel">
        ...
        {isAiLoading ? (
          ...
          <div className="feedback-sections">
            {sections.map(section => (
              <div key={section.title} className="feedback-section-block">
                <span className="feedback-section-title">{section.title}</span>
                <p className="feedback-section-body">{section.content}</p>
              </div>
            ))}
          </div>
        ) : (
      ...
      <Button variant="outline" size="lg" className="results-reset-btn" onClick={onReset} type="button">
